...
| Tier allocation via Lagrangian relaxation (§2.3, §9) | **ket-opt** | WQS binary search (Aliens trick) over nested budget constraints; O(n · (log V)^k) joint calibration of cost, depth, and tier-3 limits. Tier-aware traversal prunes Skip-allocated subtrees at runtime, enforcing the parent-activation constraint. |
...
### 11.3 Decay and Entropy Demonstration: [`decay_entropy.ipynb`](https://github.com/nickjoven/jfk-dsa/blob/main/decay_entropy.ipynb)

A Python notebook demonstrating the activation decay and localized amplitude entropy primitives on an 11-node directed epistemic graph with three semantic clusters and two bridge nodes at structural holes. It covers per-node exponential decay parameterized by half-life, normalized Shannon entropy of decay-adjusted neighbor activation distributions as a traversal scoring signal, composition of decay and entropy (distinguishing quiescent from contested nodes), and contradiction detection via entropy elevation at bridge nodes receiving simultaneously-active contradictory inputs.
